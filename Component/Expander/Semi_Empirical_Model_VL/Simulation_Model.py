...
class Expander_SE:
    ...
    def update_inputs(self, point_su, point_ex, N_exp, T_amb):
    ...
    def check_calculable(self):
        ...
        self.Required_inputs = [self.point_su.p, self.point_su.h, self.point_ex.p, self.N_rot, self.T_amb]

        if all(Input is not None for Input in self.Required_inputs):
                self.calculable = True
        
    def check_parametrized(self):
        if self.AU_amb != None and self.AU_su_n != None and self.AU_ex_n != None and self.A_leak != None and self.d_su1 != None and self.m_dot_n != None and self.W_dot_loss_0 != None and self.alpha != None and self.C_loss != None and self.rv_in != None and self.V_s != None:
    ...
    def System(self, x):
        ...
        if self.N_rot != None and self.point_su.m_dot != None or self.point_su.m_dot != None:
            # If the rotational speed and the mass flow rate are inputs of the model
            self.T_w = x[0]
            self.m_dot = self.point_su.m_dot
        
        elif self.N_rot != None:
    ...
    def solve(self):
        ...
        if self.calculable and self.parametrized:
            ...
            stop = 0
            if self.point_su.completely_known and self.N_rot != None:
                j = 0
                
                while not stop and j < len(x_T_guess):
                    # Loop to permit multiple attempts to solve the implicit calculation 
                    T_w_guess = x_T_guess[j]*self.point_su.T+(1-x_T_guess[j])*self.T_amb #initial value for T_wall
                    #---------------------------------------------------------------------
                    args = ()
                    x = [T_w_guess]
                    #--------------------------------------------------------------------------
                    ...
                    # To unsure that we didn't converge to a local minimum
                    if res_norm < 1e-4: #flag > 0 
                        stop = 1
                    j = j + 1
            
            elif self.point_su.state_known and self.N_rot != None:
                j = 0
                while not stop and j < len(x_T_guess):
                    k = 0
                    while not stop and k < len(ff_guess):
                        ...
                        # Loop to permit multiple attempts to solve the implicit calculation 
                        m_dot_guess = ff_guess[k]*self.V_s*self.N_rot/60*PropsSI('D', 'P', self.point_su.p, 'H', self.point_su.h, self.point_su.fluid) #initial value for M_dot
                        T_w_guess = x_T_guess[j]*self.point_su.T+(1-x_T_guess[j])*self.T_amb #initial value for T_wall
                        #---------------------------------------------------------------------
                        args = ()
            ...
            self.point_ex.set_m_dot(self.m_dot)
            self.point_ex.set_h(self.h_ex)
            # self.point_ex.set_p(self.P_ex)
            self.defined = True
